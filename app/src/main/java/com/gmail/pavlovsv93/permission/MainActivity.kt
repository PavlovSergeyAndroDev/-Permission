...
const val REQUEST_CODE = 42 // Код результата ответа, ожидаемый при получении подтверждения (задается разработчиком)
        ...
        applicationContext?.let {
            when{
                ...
                shouldShowRequestPermissionRationale(Manifest.permission.READ_CONTACTS) ->{ // Вызов пояснения, для чего нужен доступ
                    AlertDialog.Builder(this@MainActivity).setTitle("Доступ к контактам")                  // Задать заголовок окна пояснения
